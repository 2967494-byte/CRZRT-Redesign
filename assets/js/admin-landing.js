...
(function () {
  ...
  function migrateMainPageData(raw) {
    ...
    if (!Array.isArray(data.heroSlides) || !data.heroSlides.length) {
      ...
      data.heroSlides = [{
        ...
        background: bg,
        link: (raw && raw.heroLink !== undefined) ? raw.heroLink : (DEFAULT_LANDING_MAIN.heroSlides[0].link || '')
      }];
    } else {
      data.heroSlides = data.heroSlides.map(function(s) {
        return Object.assign({}, s, {
          link: s && s.link !== undefined ? s.link : ''
        });
      });
    }
  ...
  function collectHeroSlidesFromForm() {
    ...
    for (var i = 0; i < heroCount; i++) {
      var _t, _tc, _tt, _tl, _tfs, _tfw, _ti, _tu, _s, _sc, _st, _sl, _sfs, _sfw, _si, _su, _link;
      heroSlides.push({
        ...
        link: (((_link = document.getElementById("m_hero_link_".concat(i))) === null || _link === void 0 ? void 0 : _link.value) || '').trim()
      });
    }
    return heroSlides;
  }
  ...
  function collectMainPageFromForm(mainPageData) {
    ...
    for (var i = 0; i < heroCount; i++) {
      ...
      heroSlides.push({
        ...
        background: readImageVal("m_hero_bg_".concat(i)),
        link: (((_link2 = document.getElementById("m_hero_link_".concat(i))) === null || _link2 === void 0 ? void 0 : _link2.value) || '').trim()
      });
    }
    var serviceCards = [];
    var svcCount = document.querySelectorAll('[id^="m_svc_title_"]').length;
